...
void CDataManager::processData_phone()
{
	...
	// send message
	::PostMessage(GlobalData::Instance()->getMainHwnd(),WM_MCG_DATA_GET_PHONE_RESULT,0,isSuccess);
...
void CDataManager::processData_game_family()
{
	...
	if( !reader.parse(jsonData,json_object) )
	{
		::PostMessage(GlobalData::Instance()->getMainHwnd(),WM_MCG_DATA_GET_GAME_FAMILY_RESULT,0,0);
		return;
	}
	...
	{
		::PostMessage(GlobalData::Instance()->getMainHwnd(),WM_MCG_DATA_GET_GAME_FAMILY_RESULT,0,1);
	}
}
...
void CDataManager::processData_game()
{
	...
	{
		::PostMessage(GlobalData::Instance()->getMainHwnd(),WM_MCG_DATA_GET_GAME_RESULT,0,1);
	}
}
