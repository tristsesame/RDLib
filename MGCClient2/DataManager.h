...
public:
	...
	void getLoginData(mgcHttpLoginResult &result) {result = m_dataLoginResult;};

	//phone
	bool requestPhone();
	void getPhoneData(mgcHttpPhoneResult &result) {result = m_dataPhoneResult;};

	//区取客户端二维码
	bool requestQRCode();
	void getQRCodeResult(mgcHttpQRCodeResult &result) {result = m_dataQRCodeResult;}

	//取下载文件 
	bool requestDownFile(mgcHttpDownFile &downFile);
	void getDownFileResult(mgcHttpDownFileResult &result) {result = m_dataDownFileResult;}

	//取游戏分类
	bool requestAllGameFamily();
	void getGameFamilyResult(mgcHttpGameFamilyResult &result) {result = m_dataGameFamilyResult;}

	//取游戏列表信息
	bool requestAllGame();
	void getGameResult(mgcHttpGameResult &result) {result = m_dataGameResult;}
